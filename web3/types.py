...
TxData = TypedDict('TxData', {'accessList': AccessList, 'blobVersionedHashes': Sequence[HexBytes], 'blockHash': HexBytes, 'blockNumber': BlockNumber, 'chainId': int, 'data': Union[bytes, HexStr], 'from': ChecksumAddress, 'gas': int, 'gasPrice': Wei, 'maxFeePerBlobGas': Wei, 'maxFeePerGas': Wei, 'maxPriorityFeePerGas': Wei, 'hash': HexBytes, 'input': HexBytes, 'nonce': Nonce, 'r': HexBytes, 's': HexBytes, 'to': ChecksumAddress, 'transactionIndex': int, 'type': Union[int, HexStr], 'v': int, 'value': Wei, 'yParity': int}, total=False)
TxParams = TypedDict('TxParams', {'accessList': AccessList, 'blobVersionedHashes': Sequence[Union[str, HexStr, bytes, HexBytes]], 'chainId': int, 'data': Union[bytes, HexStr], 'from': Union[Address, ChecksumAddress, str], 'gas': int, 'gasPrice': Wei, 'maxFeePerBlobGas': Union[str, Wei], 'maxFeePerGas': Union[str, Wei], 'maxPriorityFeePerGas': Union[str, Wei], 'nonce': Nonce, 'to': Union[Address, ChecksumAddress, str], 'type': Union[int, HexStr], 'value': Wei}, total=False)
WithdrawalData = TypedDict('WithdrawalData', {'index': int, 'validator_index': int, 'address': ChecksumAddress, 'amount': Gwei})
...
EthSubscriptionParams = Union[BlockTypeSubscriptionResponse, TransactionTypeSubscriptionResponse, LogsSubscriptionResponse, SyncingSubscriptionResponse, GethSyncingSubscriptionResponse]
...
class FormattedEthSubscriptionResponse(TypedDict):
    subscription: HexStr
    result: Union[BlockData, TxData, LogReceipt, SyncProgress, GethSyncingSubscriptionResult]

class CreateAccessListResponse(TypedDict):
    accessList: AccessList
    gasUsed: int
Middleware = Callable[[Callable[[RPCEndpoint, Any], RPCResponse], 'Web3'], Any]
AsyncMiddlewareCoroutine = Callable[[RPCEndpoint, Any], Coroutine[Any, Any, RPCResponse]]
AsyncMiddleware = Callable[[Callable[[RPCEndpoint, Any], RPCResponse], 'AsyncWeb3'], Any]
...
CallOverrideParams = TypedDict('CallOverrideParams', {'balance': Optional[Wei], 'nonce': Optional[int], 'code': Optional[Union[bytes, HexStr]], 'state': Optional[Dict[HexStr, HexStr]], 'stateDiff': Optional[Dict[HexStr, HexStr]]}, total=False)
CallOverride = Dict[ChecksumAddress, CallOverrideParams]
GasPriceStrategy = Union[Callable[['Web3', TxParams], Wei], Callable[['AsyncWeb3', TxParams], Wei]]
TxReceipt = TypedDict('TxReceipt', {'blockHash': HexBytes, 'blockNumber': BlockNumber, 'contractAddress': Optional[ChecksumAddress], 'cumulativeGasUsed': int, 'effectiveGasPrice': Wei, 'gasUsed': int, 'from': ChecksumAddress, 'logs': List[LogReceipt], 'logsBloom': HexBytes, 'root': HexStr, 'status': int, 'to': ChecksumAddress, 'transactionHash': HexBytes, 'transactionIndex': int, 'type': int})
...
class Uncle(TypedDict):
    ...
    transactionsRoot: HexBytes
    uncles: Sequence[HexBytes]
PendingTx = TypedDict('PendingTx', {'blockHash': HexBytes, 'blockNumber': None, 'from': ChecksumAddress, 'gas': HexBytes, 'maxFeePerGas': HexBytes, 'maxPriorityFeePerGas': HexBytes, 'gasPrice': HexBytes, 'hash': HexBytes, 'input': HexBytes, 'nonce': HexBytes, 'to': ChecksumAddress, 'transactionIndex': None, 'value': HexBytes}, total=False)
...
class LogsSubscriptionArg(TypedDict, total=False):
    address: Union[Address, ChecksumAddress, ENS, Sequence[Union[Address, ChecksumAddress, ENS]]]
